...
\begin{enumerate}[nosep]
    ...
    \item \textbf{数据库索引缺失}：新增4个复合索引优化消息会话查询（from\_user\_id+to\_user\_id+created\_at）、未读计数（to\_user\_id+is\_read）、收藏排序（user\_id+created\_at）和卖家商品筛选（user\_id+status）。
